...
public class Programa {
	...
	public static void main(String[] args) throws Exception {
		 ...
		 * Consultas XPath 
		 * - Todas las notas 
		 * * consulta = xpath.compile("//nota");
		 * - Las notas de las calificaciones (excluidas las diligencias) 
		 * * consulta = xpath.compile("//calificacion/nota");
		 * * consulta = xpath.compile("/acta/calificacion/nota");
		 * - Todas las fechas de las diligencias 
		 * * consulta = xpath.compile("//diligencia/@fecha"); con @ pq es un atributo
		 * - Diligencias que tengan establecido el atributo "extraordinaria".
		 * * consulta = xpath.compile("//diligencia[@extraordinaria]"); 
		 - Diligencias que NO tengan establecido el atributo "extraordinaria".
		 * * consulta = xpath.compile("//diligencia[not @extraordinaria]");
		 * - Calificaciones con nota >= 9. 
		 * * consulta = xpath.compile("//calificaciones[nota>=9]");
		 * - Calificaciones o diligencias para el NIF "13322156M" 
		 * * consulta = xpath.compile("//calificaciones[nif='13322156M'] | //diligencia[nif='13322156M'] ");
		 * - Calificaciones cuyo NIF comience por "2". 
		 * * consulta = xpath.compile("//calificacion[starts-with(nif,'2')]");
		 * - La segunda calificación
		 * * consulta = xpath.compile("//calificacion[2]");
		 **/
